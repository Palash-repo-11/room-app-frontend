...
const MeetingRoom = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (res.data.success && userInfo) {
          setIsValid(true);
          setOwnerId(res.data.meeting.ownerId); // use correct field from backend response
          ...
          socketRef.current.emit('join-room', {
            ...
            userId: userInfo.id,
            userName: userInfo.name,
            isOwner: userInfo.id === res.data.meeting.ownerId,
          });
          ...
          socketRef.current.on('receive-message', ({ userId, userName, message, timestamp }) => {
  ...
  const handleSendMessage = () => {
    if (chatInput.trim()) {
      socketRef.current.emit('send-message', { message: chatInput });
      setMessages(prev => [
        ...prev,
        {
          userId: userInfo.id,
